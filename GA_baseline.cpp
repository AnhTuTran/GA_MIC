...
*/
#pragma offload_attribute(push, target(mic: 0))
...
// Launch  init
	...
	#pragma offload target(mic:0) inout(mang_NST : length(sizePop), mang_fitness : length(sizePop) ALLOC) {
...
// move top NST
	...
	// copy to parentsNST
	for (int i =0 ; i< sizePop*numTask ; i++) { mang_NST[i] = temNST[i]; NSToffspring[i] = temNST[i];}
	// copy to parents fitness
	for (int i =0 ; i< sizePop ; i++) { mang_fitness[i] = keySort[i]; fitness_offspring[i] = keySort[i];}
}
